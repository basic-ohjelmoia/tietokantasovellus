...
/**
 * Malli Komponentti-taulun merkinnöille.
 */
public class Komponentti {
    ...
    /**
     * "Sijainti" on väliaikainen luonteeltaan väliaikainen tieto, jota EI
     * tallenneta tietokannan Komponentti-tauluun. Sijaintia hyödynnetään
     * lähinnä silloin, kun yksittäisestä Mechistä ja sen komponenteista
     * esitetään tietoja käyttäjälle. Sijainti on myös tärkeä simuloidun
     * taistelun sujuvan toiminnan kannalta.
     *
     * @param sijainti Sijainti johon komponentti on asennettu
     */
    ...
    /**
     * Laskee komponentille hinnan sen ominaisuuksien perusteella.
     *
     * @return
     */
    public int getCost() {
        ...
        if (this.kategoria.equalsIgnoreCase("ASE")) {
            ...
            if (this.massa * 1.5 > this.weaponDamage) {
                hinta = hinta / 2;
            }
            if (this.massa * 1.25 > this.weaponDamage) {
                hinta = hinta / 2;
            }
            if (this.massa < this.weaponDamage) {
                hinta = hinta * 2;
            }
            if (this.massa * 1.5 < this.weaponDamage) {
                hinta = hinta * 2;
            }
            if (this.massa * 2 < this.weaponDamage) {
                hinta = hinta * 2;
            }
            hinta = hinta + ((21 - this.heat) * 50);
            hinta = hinta + ((25 - this.weaponMaxRange) * 25);
            if (this.weapontype.equalsIgnoreCase("KINETIC")) {
                hinta = hinta / 3;
                hinta += (hinta / 6) * this.weaponAmmo;
            }
            if (this.weapontype.equalsIgnoreCase("MISSILE")) {
                hinta = (hinta / 2) + 1000;
                hinta += (hinta / 5) * this.weaponAmmo;
            }
            if (this.weapontype.equalsIgnoreCase("MELEE")) {
                hinta = (hinta * 2) + 2000;
            }
            if (this.weaponAmmo == 0) {
                hinta = (hinta * 2) + 500;
            } else {
                hinta -= (hinta / (2 + this.weaponAmmo));
            }
        } else {
            ...
            if (this.varusteType.equalsIgnoreCase("COCKPIT")) {
                hinta = hinta * 25;
            }
            if (this.varusteType.equalsIgnoreCase("GYROSCOPE")) {
                hinta = hinta * 10;
            }
            if (this.varusteType.equalsIgnoreCase("SENSORS")) {
                hinta = hinta * 5;
            }
            if (this.varusteType.equalsIgnoreCase("ACTIVE CAMO")) {
                hinta = hinta * 75;
            }
            if (this.varusteType.equalsIgnoreCase("TARGETTING COMPUTER")) {
                hinta = hinta * 15;
            }
            if (this.varusteTier == 1) {
                hinta += 5000;
            }
            if (this.varusteTier == 2) {
                hinta += 1500;
            }
            if (this.varusteTier == 3) {
                hinta = hinta / 2;
            }
            if (this.massa == 1) {
                hinta += 2500;
    ...
    /**
     * Päivittää VARUSTE-tyyppisen komponentin ominaisuudet tietokantaan, mutta
     * vain niiltä osin, kun ominaisuuksia on muutettu.
     *
     * Muistilista: equipmentid, equipmentname,equipmenttype, weight,
     * equipmenttier, equipmentactivity, heat, volume, location);
     *
     * @param idstring Komponentin id-numero stringinä
     * @param nimi komponentin nimi
     * @param tyyppi tyyppi
     * @param weight paino
     * @param strtier varusteen tier
     * @param activity varusteen activity
     * @param strheat kuumuusarvo
     * @param volume kokoluokka
     * @param location sallitut sijainnit
     * @throws SQLException
    ...
    public static void paivitaVarusteKomponentti(String idstring, String nimi, String tyyppi, String weight, String strtier, String activity, String strheat, String volume, String location) throws SQLException, NamingException {
        ...
        if (komponentti == null) {
            komponentti = getKomponentti(0, true);
            id = getUusiID();
            uusiKomponentti = true;
        }

        if (nimi.equalsIgnoreCase("keep")) {
            nimi = komponentti.getNimi();
        }

        if (nimi.length() > 39) {
            nimi = nimi.substring(0, 38);
            if (!Tarkistaja.onkoAlfanumeerinen(nimi)) {
                nimi = null;
                nimi = "";
            }
        }
        ...
        nimi = null;
        nimi = "";  // komponenttien nimeäminen käyttäjän taholta estetty, annetaan sovelluksen nimetä komponentit automaattisesti

        if (tyyppi.equalsIgnoreCase("keep")) {
            tyyppi = komponentti.getVarustetype();
        }
        if (weight.equalsIgnoreCase("keep")) {
            massa = komponentti.getMassa();
        } else {
            massa = Integer.parseInt(weight);
        }
        if (strtier.equalsIgnoreCase("keep")) {
            tier = komponentti.getVarustetier();
        } else {
            tier = Integer.parseInt(strtier);
        }
        if (strheat.equalsIgnoreCase("keep")) {
            heat = komponentti.getHeat();
        } else {
            heat = Integer.parseInt(strheat);
        }
        if (activity.equalsIgnoreCase("keep")) {
            activity = komponentti.getVarusteactivity();
        }
        if (volume.equalsIgnoreCase("keep")) {
            volume = komponentti.getKokoluokka();
        }
        if (location.equalsIgnoreCase("keep")) {
            location = komponentti.getSijoituspaikka();
        }

        if (activity.contentEquals("PASSIVE") || activity.contentEquals("ACTIVE")) {
        } else {
            activity = "PASSIVE";
        }
        ...
        //heat = Tarkista.tarkistaKomponentinHeat(tyyppi, heat);  // IMPLEMENTOIMATTA 
        //massa = Tarkista.tarkistaKomponentinMassa(tyyppi, massa); // -"-
    ...
    /**
     * Päivittää ASE-tyyppisen komponentin ominaisuudet tietokantaan, mutta vain
     * niiltä osin, kun ominaisuuksia on muutettu.
     *
     * @param idstring aseen id-numero
     * @param nimi nimi
     * @param tyyppi asetyyppi
     * @param weight paino
     * @param strdamage vahinkoarvo
     * @param strheat lämpöarvo
     * @param strrange kantamaluokitus (väliltä "cqb, short, long, vlong, arty",
     * defaultoituu mediumiksi)
     * @param strammo ammuskapasiteettiluokitus
     * @param volume kokoluokka
     * @param location sallitut sijainnit
     * @throws SQLException
     * @throws NamingException
    ...
    public static void paivitaKomponentti(String idstring, String nimi, String tyyppi, String weight, String strdamage, String strheat, String strrange, String strammo, String volume, String location) throws SQLException, NamingException {
        ...
        if (tyyppi.equalsIgnoreCase("keep")) {
            tyyppi = komponentti.getWeapontype();
        }
        if (weight.equalsIgnoreCase("keep")) {
            massa = komponentti.getMassa();
        } else {
        ...
        if (strdamage.equalsIgnoreCase("keep")) {
            damage = komponentti.getWeapondamage();
        } else {
        ...
        if (strrange.equalsIgnoreCase("keep")) {
            maxrange = komponentti.getWeaponmaxrange();
            minrange = komponentti.getWeaponminrange();
        } else {
            int newMax = 0;
            int newMin = 0;
            if (strrange.equalsIgnoreCase("cqb")) {
                newMax = 1;
                newMin = 1;
            } else if (strrange.equalsIgnoreCase("short")) {
                newMax = 3;
                newMin = 1;
            } //else if (strrange.equalsIgnoreCase("med")) {newMax =5; newMin=1;}
            else if (strrange.equalsIgnoreCase("long")) {
                newMax = 9;
                newMin = 2;
            } else if (strrange.equalsIgnoreCase("vlng")) {
                newMax = 12;
                newMin = 3;
            } else if (strrange.equalsIgnoreCase("arty")) {
                newMax = 15;
                newMin = 5;
            } else {
                newMax = 5;
                newMin = 1;
            }  // DEFAULTOIDAAN MEDIUMIIN
            maxrange = newMax;
            minrange = newMin;
        }
        if (strammo.equalsIgnoreCase("keep")) {
            ammo = komponentti.getWeaponammo();
        } else {
            ammo = 0; // DEFAULT 0 = INFINITE AMMO 


            if (strammo.equalsIgnoreCase("single")) {
                ammo = 1;
            } else if (strammo.equalsIgnoreCase("limited")) {
                ammo = 5;
            } else if (strammo.equalsIgnoreCase("standard")) {
                ammo = 10;
            } else if (strammo.equalsIgnoreCase("extended")) {
                ammo = 15;
            } else if (strammo.equalsIgnoreCase("plentiful")) {
                ammo = 25;
            } else {
                ammo = 0;
            }
        ...
        if (minrange < 1 || minrange > 5) {
            minrange = 1;
        }
        if (maxrange < 1 || maxrange > 15) {
            maxrange = 5;
        }
        if (heat < 0 || heat > 30) {
            heat = 1;
        ...
        if (tyyppi.equalsIgnoreCase("MELEE")) {
            heat = 0;
            damage = 1;
            minrange = 1;
            maxrange = 1;
            ...
            if (volume.equalsIgnoreCase("LARGE")) {
                massa = 7;
                damage = 13;
            }
            if (volume.equalsIgnoreCase("XL")) {
                massa = 9;
                damage = 17;
            }
        }
    ...
    /**
     * Poistaa komponentin tietokannasta. Ei siis tiettyyn mechiin asennettua
     * komponenttia, vaan koko komponentti-lajin.
     *
     * @param komponentti_id poistettavan komponentin id
     * @throws NamingException
     * @throws SQLException
     */
    ...
    /**
     * Muodostaa ASE-tyyppiselle komponentille nimen
     *
     * @return
     */
    public String getAseCommonName() {
        ...
        if (getWeapontype().equalsIgnoreCase("MISSILE")) {
            commonName = "Missile";
            if (this.weaponAmmo == 1 && this.weaponDamage >= 10) {
                commonName = "Stand-Off Missile";
            }

            if (this.weaponMaxRange >= 12) {
                etuliite = "Long Range";
            } else if (this.weaponMaxRange >= 7) {
                etuliite = "Medium Range";
            } else {
                etuliite = "Short Range";
            }
        ...
        if (getWeapontype().equalsIgnoreCase("Kinetic")) {
            ...
            if (this.massa >= 12 && this.weaponMaxRange >= 12) {
                commonName = "Gauss Rifle";
            }
            if (this.massa >= 10 && this.weaponMaxRange >= 12 && this.weaponAmmo == 0) {
                commonName = "Rail Gun";
            }
        ...
        if (getWeapontype().equalsIgnoreCase("AUTO")) {
            ...
            if (this.massa >= 10) {
                commonName = "Chain Gun";
            }
            if (this.weaponAmmo == 1) {
                commonName = "Bullet Storm";
            }

            if (this.weaponAmmo == 0) {
                commonName = "Pulse Laser";
            }
            if (this.weaponAmmo == 0 && this.massa >= 12) {
                commonName = "Laser Array";
            }

            if (this.weaponDamage >= 8) {
                etuliite = "Heavy";
            } else if (this.weaponDamage >= 5) {
                etuliite = "Medium";
            } else {
                etuliite = "Light";
            }
        ...
        if (getWeapontype().equalsIgnoreCase("MELEE")) {
            commonName = "Hatchet";
            if (this.kokoluokka.equalsIgnoreCase("SMALL")) {
                etuliite = "Light";
            }
            if (this.kokoluokka.equalsIgnoreCase("MEDIUM")) {
                etuliite = "Medium";
            }
            if (this.kokoluokka.equalsIgnoreCase("LARGE")) {
                etuliite = "Heavy";
            }
            if (this.kokoluokka.equalsIgnoreCase("XL")) {
                etuliite = "Assault";
            }
...
//       else {etuliite="Light";}
        ...
        if (getWeapontype().equalsIgnoreCase("ENERGY")) {
            ...
            if (this.massa >= 10 && this.weaponDamage >= 10) {
                ...
                if (this.massa >= 15 || this.weaponDamage >= 15) {
                    etuliite = "Heavy";
                } else {
                    etuliite = "";
                }

                if (this.weaponMaxRange >= 15) {
                    etuliite = "ER";
                }
                if (this.weaponMaxRange <= 9) {
                    etuliite = "CQB";
                }
            } else {
    ...
    /**
     * Muodostaa varuste-tyyppiselle komponentille nimen
     *
     * @return
     */
    public String getVarusteCommonName() {
        ...
        if (this.varusteType.contains("HEAT SINK")) {
            commonName = "Heat Sink";
        }
        if (this.varusteType.contains("JUMP JET")) {
            commonName = "Jump Jet";
        }
        if (this.varusteType.contains("ANTI MISSILE SYSTEM")) {
            commonName = "Anti-Missile System";
            hightech = true;
        }
        if (this.varusteType.contains("ARMOR PLATING")) {
            commonName = "Armor Plating";
        }
        if (this.varusteType.contains("GYROSCOPE")) {
            commonName = "Gyroscope";
            hightech = true;
        }
        if (this.varusteType.contains("COCKPIT")) {
            commonName = "Cockpit";
            hightech = true;
        }
        if (this.varusteType.contains("TARGETTING COMPUTER")) {
            commonName = "Targetting Computer";
            hightech = true;
        }
        if (this.varusteType.contains("ACTIVE CAMO")) {
            commonName = "Active Camo";
            hightech = true;
        }
        if (this.varusteType.contains("SENSORS")) {
            commonName = "Sensor Package";
            hightech = true;
        }
        if (this.varusteType.contains("ACTUATORS")) {
            commonName = "Actuators";
        }

        String etuliite = "Light";
        if (hightech) {
            etuliite = "Minor";
            if (this.varusteTier < 3) {
                etuliite = "Miniaturized";
            }
        }
        if (this.varusteType.contains("ACTUATORS")) {
            if (this.kokoluokka.equalsIgnoreCase("SMALL")) {
            ...
            if (this.kokoluokka.equalsIgnoreCase("HEAVY")) {
                etuliite = "Heavy";
            }
            if (this.kokoluokka.equalsIgnoreCase("XL")) {
                etuliite = "XL";
    ...
    /**
     * Muodostaa komponentille lyhenne-nimen
     *
     * @return
     */
    public String getLyhenne() {
        ...
        if (this.kategoria.equalsIgnoreCase("ASE")) {
            if (getMassa() >= 15) {
                lyhenne = "X";
            } else if (getMassa() >= 8 || getKokoluokka().contains("LARGE") || getKokoluokka().contains("XL")) {
                lyhenne = "H";
            } else if (getMassa() >= 5 || getKokoluokka().contains("MEDIUM")) {
                lyhenne = "M";
            } else if (getMassa() <= 3) {
                lyhenne = "S";
            }
            ...
            if (getWeapontype().equalsIgnoreCase("KINETIC")) {
                if (getMassa() <= 10 || getWeapondamage() <= 20) {
                    lyhenne += "AC";
                } else {
                    lyhenne = "RAIL";
                }
            ...
            if (getWeapontype().equalsIgnoreCase("MISSILE")) {
                ...
                if (getWeaponmaxrange() <= 6) {
                    lyhenne = "S" + lyhenne;
                } else if (getWeaponmaxrange() <= 9) {
                    lyhenne = "M" + lyhenne;
                } else {
                    lyhenne = "L" + lyhenne;
                }

                if (getWeapondamage() > 20) {
                    lyhenne = "SO-MSL-";
                }
            }
            if (getWeapontype().equalsIgnoreCase("MELEE")) {
                ...
                if (getHeat() >= 10) {
                    ...
                    lyhenne += "C";
                } else if (getWeapondamage() >= 12) {
                    lyhenne += "X";
                } else if (getWeapondamage() < 5) {
                    lyhenne += "P";
                } else if (getWeaponmaxrange() >= 12) {
                    lyhenne += "E";
                } else {
                    lyhenne += "A";
                }
            ...
            if (lyhenne.length() < 1) {
                lyhenne += this.varusteType.charAt(0);
            }

            int mallinumero = (this.varusteType.length() * (100 / (this.massa)) * (this.heat + 1));

            if (this.varusteTier == 1) {
                lyhenne = lyhenne + "-" + mallinumero;
            }
            if (this.varusteTier == 2) {
                lyhenne = lyhenne + "-" + (mallinumero / 3);
            }
            if (this.varusteTier == 3) {
                lyhenne = lyhenne + "-" + (mallinumero / 7);
            }
            ...
            if (getMassa() >= 15) {
                ...
                lyhenne = lyhenne + "C";
            } else if (this.varusteActivity.equalsIgnoreCase("ACTIVE")) {
                lyhenne = lyhenne + "B";
            } else {
                lyhenne = lyhenne + "A";
            }
            ...
            if (this.varusteTier == 1) {
                lyhenne = lyhenne + this.komponentti_id + "-SR";
            }
            if (this.varusteTier == 2) {
                lyhenne = lyhenne + this.komponentti_id + "-R";
            }
            if (this.varusteTier == 3) {
                lyhenne = lyhenne + this.komponentti_id;
            }
    ...
    /**
     * Palauttaa ASE-tyyppisen komponentti-olion tietokannasta
     *
     * @param komponentti_id
     * @return
     * @throws NamingException
     * @throws SQLException
     */
    ...
    /**
     * Hakee tietokannasta Komponentin. Jos haetaan id-numeroa 0, metodi
     * palauttaa pohjan uudelle komponentille. id=0:lla Boolean equipment=false
     * tuottaa aseen, equipment=true varusteen. Jos id-numero on muu kuin nolla,
     * boolean-valinta sivuutetaan.
     *
...
//  String sql = "SELECT komponentti_id, nimi, massa, kokoluokka, heat, kategoria, sijoituspaikka, weapon_damage, weapon_maxrange, weapon_minrange, weapon_type, weapon_ammo FROM komponentti WHERE komponentti_id="+komponentti_id;
    ...
    /**
     * Palauttaa kaikki komponentit tietokannasta tyypistä riippumatta
     *
     * @return
     * @throws NamingException
     * @throws SQLException
    ...
    /**
     * Palauttaa listan varuste-tyyppisistä komponenteista
     *
     * @return
     * @throws NamingException
     * @throws SQLException
...
//  kayttajat.add(new Kayttaja(999, "none", "nopw"));
    ...
    /**
     * Palauttaa olio-luettelona kaikki ase-komponentit aseen tyypistä
     * riippumatta
     *
     * @return
     * @throws NamingException
    ...
    /**
     * Palauttaa luettelon aseista, joiden weapon_type vastaa kutsuttua. Jos
     * weapon_typeksi annetaan "ALL" palautetaan kaikki ase-komponentit tyypistä
     * riippumatta
     *
     * @param weapon_type Weapon_typen nimi
     * @return
     * @throws NamingException
     * @throws SQLException
     */
    public static List<Komponentti> getAseKomponentit(String weapon_type) throws NamingException, SQLException {
        String sqltype = "";
        if (weapon_type.equalsIgnoreCase("ALL")) {
        } else {
...
//  kayttajat.add(new Kayttaja(999, "none", "nopw"));
    ...
    /**
     * Super-räyhäkkäästi koodattu metodi, joka etsii ensimmäisen vapaan
     * id-numeron...
     *
     * @return vpaaa id
     * @throws NamingException
    ...
    /**
     * Palauttaa kaikki komponentit listana
     *
     * @return
     * @throws NamingException
     * @throws SQLException
...
//  kayttajat.add(new Kayttaja(999, "none", "nopw"));
    ...
    public int getAsennuskerrat() throws NamingException, SQLException {
        List<Komponentisto> komponentisto = Komponentisto.getKomponentisto();
        int komponentit = 0;
        for (Komponentisto yksilo : komponentisto) {
            if (yksilo.getComponent_id() == this.komponentti_id) {
                komponentit++;
            }
        }
