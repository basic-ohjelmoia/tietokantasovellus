...
/**
 * Malli Reaktori-taulun merkinnoille eli mechien toiminnan kannalta
 * välttämättömille reaktoreille Oliona Reaktorit extendoivat
 * Komponentti-luokkaa, joten sovelluksen useimpien toimintojen kannalta
 * reaktoreita voidaan käsitellä samanlaisissa yhteyksissä kuin varsinaisia
 * komponentteja.
 */
public class Reaktori extends Komponentti {
    ...
    /**
     * Päivittää Reaktorin tiedot tietokantaan, mutta vain niiltä osin kuin
     * muutoksia on tapahtunut
     *
     * @param idstring reaktorin id-numero stringinä
     * @param nimi reaktorin nimi
     * @param strcooling reaktorin cooling-arvo stringinä
     * @param strteho reaktorin power-arvo stringinä
     * @param strmassa reaktorin massa stringinä
     * @throws SQLException
    ...
    public static void paivitaReaktori(String idstring, String nimi, String strcooling, String strteho, String strmassa) throws SQLException, NamingException {
        ...
        if (reaktori == null) {
            reaktori = getReaktori(0);
            id = getUusiID();
            uusiReaktori = true;
        }

        if (nimi.equalsIgnoreCase("keep")) {
            nimi = reaktori.getNimi();
        }

        if (nimi.length() > 39) {
            nimi = nimi.substring(0, 38);
        }
        if (strcooling.equalsIgnoreCase("keep")) {
            cooling = reaktori.getCooling();
        } else {
            cooling = Integer.parseInt(strcooling);
        }
        if (strmassa.equalsIgnoreCase("keep")) {
            massa = reaktori.getMassa();
        } else {
            massa = Integer.parseInt(strmassa);
        }
        if (strteho.equalsIgnoreCase("keep")) {
            teho = reaktori.getTeho();
        } else {
            teho = Integer.parseInt(strteho);
        }
    ...
    /**
     * Poistaa reaktorin tietokannasta
     *
     * @param reaktori_id
     * @throws NamingException
     * @throws SQLException
    ...
    public String getReaktoriCommonName() {
        ...
        String etuliite = "";
        if (this.teho > 300) {
            commonName = "Fusion Reactor";
        } else if (this.teho >= 200) {
            commonName = "Fission Reactor";
        } else {
            commonName = "ICE Reactor";
        }
        ...
        if (cooling >= 12) {
            etuliite = etuliite + "Cold ";
        } else if (cooling < 8) {
            etuliite = etuliite + "Red ";
        }
    ...
    public String getLyhenne() {
        ...
        if (cooling >= 15) {
            ...
            takaliite = "H";
        } else if (cooling == 10 && this.teho > 250) {
            takaliite = "B";
        } else {
            takaliite = "A";
        }
...
//        return laskuri;
    ...
    public String getKokoluokka() {
        if (this.massa >= 10) {
            return "XL";
        } else if (this.massa >= 7) {
            return "LARGE";
        } else if (this.massa >= 3) {
            return "MEDIUM";
        }
        return "SMALL";
    }
    ...
    public int getEfficiency() {
        return (this.teho / this.massa);
    }

    public int getAsennuskerrat() throws NamingException, SQLException {
        List<Mech> mechit = Mech.getMechit();
        int komponentit = 0;
        for (Mech yksilo : mechit) {
            if (yksilo.getReaktori().getReaktori_id() == this.reaktori_id) {
                komponentit++;
            }
        }
